...
public class SM2SignFactory {
    ...
    public static SM2SignFactory getInstance() {
        return new SM2SignFactory();
    }

    public SM2SignFactory() {
    ...
    public byte[] sm2GetZ(byte[] userId, ECPoint userKey) {
        ...
        byte[] p = BaseUtil.byteConvert32Bytes(this.a);
        sm3.update(p, 0, p.length);

        p = BaseUtil.byteConvert32Bytes(this.b);
        sm3.update(p, 0, p.length);

        p = BaseUtil.byteConvert32Bytes(this.gx);
        sm3.update(p, 0, p.length);

        p = BaseUtil.byteConvert32Bytes(this.gy);
        sm3.update(p, 0, p.length);

        p = BaseUtil.byteConvert32Bytes(userKey.normalize().getXCoord().toBigInteger());
        sm3.update(p, 0, p.length);

        p = BaseUtil.byteConvert32Bytes(userKey.normalize().getYCoord().toBigInteger());
        sm3.update(p, 0, p.length);
     ...
     * @param sm2Result
     */
    public void sm2Sign(byte[] md, BigInteger userD, ECPoint userKey, SM2SignResult sm2Result) {
    ...
    public void sm2Verify(byte md[], ECPoint userKey, BigInteger r,
                          BigInteger s, SM2SignResult sm2Result) {
        sm2Result.R = null;
        BigInteger e = new BigInteger(1, md);
