...
public class BaseUtil {
  ...
  public static int[] bytes2Ints(byte[] bytes) {
    ...
    for (int i = 0; i < bytes.length; i++) {
      ints[i] = BaseUtil.toUnsignedByte(bytes[i]);
    }
  ...
  /**
   * 把int[4](int表示无符号的bytes)转成longs[4])(无符号int)
   */
  public static long[] unsignedBytes2UnsignedInts(int[] ints) {
    byte[] bytes = BaseUtil.ints2Bytes(ints);
    return BaseUtil.bytes2UnsignedInts(bytes);
  }
  ...
  public static long unsignedBytes2UnsignedInt(int[] ints) {
    return ((long) ints[0] & 0xff) | (((long) ints[1] & 0xff) << 8) | (((long) ints[2] & 0xff)
        << 16) | (((long) ints[3] & 0xff) << 24);
  }
  ...
  public static int[] unsignedInt2UnsignedBytes(long l) {
    ...
    int[] ints = new int[4];
    ints[0] = toUnsignedByte(bytes[0]);
    ints[1] = toUnsignedByte(bytes[1]);
    ints[2] = toUnsignedByte(bytes[2]);
    ints[3] = toUnsignedByte(bytes[3]);
    return ints;
  }
